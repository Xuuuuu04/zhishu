...
**在一个精心设计的黑色面板里，统一管理 Claude Code / Codex / Gemini CLI / Qwen / OpenCode / GLM / MiniMax 等 7 款主流 AI 编程工具**
...
- [功能深度讲解](#功能深度讲解)
  ...
  - [3. Provider 系统（GLM / MiniMax）](#3-provider-系统glm--minimax)
...
- 想切换到 GLM 或 MiniMax 这种 Anthropic 兼容 Provider，还得记一堆 shell 函数
...
- 阿里的 **Qwen Code**
- 开源的 **OpenCode**
- 智谱 **GLM**、MiniMax 等**Anthropic 兼容 Provider**
...
| 特性 | 描述 |
|---|---|
| **🧠 7 款 AI 工具一键启动** | Claude Code / Codex / Gemini CLI / Qwen Code / OpenCode / GLM / MiniMax，点按钮即启动，无需记命令 |
...
| **🌐 Provider 系统** | 通过环境变量注入，让官方 Claude 二进制变身 GLM/MiniMax Provider。设置窗口可视化配置 API Key，无需 shell 函数 |
...
应用内置 **7 个主流 AI CLI 的启动配置**，每个都有：
- 唯一的品牌色（Claude 橙 / Codex 绿 / Gemini 蓝 / Qwen 青 / OpenCode 橙红 / GLM 紫 / MiniMax 粉）
...
这条命令让**官方 Claude 二进制完全变成 GLM 的 CLI**。智谱 / MiniMax 也是同样的原理。
...
**B. 扫描全部模式**：
- **递归扫描父目录下所有 git 仓库**（深度 4，自动跳过 node_modules 等）
...
**A:** 设置里有 Light 模式选项，但标记为"开发中"。目前只影响根背景层。完整的浅色支持需要把所有组件颜色迁移到 CSS 变量系统，是 Roadmap 上的一项。
